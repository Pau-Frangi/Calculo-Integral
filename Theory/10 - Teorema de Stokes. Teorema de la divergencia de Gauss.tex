...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item[$2 \implies 1$] Fijemos un $p \in U$, para cada $x \in U$ definamos $\varphi(x) = \int_{[p,x]} \vec{F}$.\\
        Veamos que $\varphi$ es un potencial para $\vec{F}$: $\forall i \in \left\{1, \cdots , n \right\}$, para ello veamos si $F_i = \frac{\partial \varphi}{\partial x_i}$.\\
        $$ \lim_{h \to 0} \frac{1}{h} \left( \int_{[p, x+h \vec{e}_i]} - \int_{[p,x]} \vec{F} \right) = \lim_{h \to 0} \frac{1}{h} \left( \varphi (x+he_i) - \varphi(x) \right)$$
        Tenemos que por $2$ si consideramos el triangulo $T$ de veritices $p$, $x$ y $x+h \vec{e}_i$, tenemos que:
        $$ \int_{[p,x] + [x,x+h \vec{e}_i] + [x+h \vec{e}_i,p]} \vec{F} = 0 \implies
        \int_{[p,x+h \vec{e}_i]} \vec{F} - \int_{[p,x]} \vec{F} = \int_{[x,x+h \vec{e}_i]} \vec{F}$$
        Luego:
        $$ \frac{1}{h} \int_{[x,x+h \vec{e}_i]} \vec{F} = \frac{1}{h} \int_{t=0}^{t=1} \langle \vec{F}(x + t h \vec{e}_i), h \vec{e}_i \rangle dt = \frac{1}{h} \cdot h \int_{t=0}^{t=1} F_i (x + t h \vec{e}_i) dt \to F_i$$
        Asi tenemos que $1 \iff 2$.
        \item[$1 \implies 3$] Se demuestra con el lema anterior.
...
\ejemplo{
    ...
    Seam $U = \{(x,y,z) \in \mathbb{R}^3 : x+y \neq 0 \}$ y $\vec{F}: U \to \mathbb{R}^3$ el campo vectorial dado por:
    ...
    El campo $\vec{F}$ es por tanto irrotacional en $U$, pero $\vec{F}$ no es conservativo:
...
\ejemplo{
    Sea el ejercicio de Nash:\\
    Encontar $X \subset \mathbb{R}^3$ tal que si denotamos:\\
    $$ V = \{ \vec{F}: \mathbb{R}^3 \setminus X \to \mathbb{R}^3 \text{Campo c^1 tal que} \quad rot(\vec{F}) = 0 \}$$
    $$ W = \{ \vec{F}: \mathbb{R}^3 \setminus X \to \mathbb{R}^3 \text{Campo c^1 tal que} \quad \vec{F}= \nabla g \text{ para algun g} \}$$
    Obteniendo que $dim(V/W)=8$ 
}