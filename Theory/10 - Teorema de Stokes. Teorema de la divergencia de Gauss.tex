...
\begin{observación}[Divergencia de un campo vectorial en $\mathbb{R}^3$]
    Sean $U \subset \mathbb{R}^3$ un abierto y $\vec{F} : U \to \mathbb{R}^3$ un campo vectorial de clase $C^1$. Se define la divergencia de $\vec{F}$ como:
    $$ div(\vec{F}) = \langle \nabla, \vec{F} \rangle = \frac{\partial F_1}{\partial x} + \frac{\partial F_2}{\partial y} + \frac{\partial F_3}{\partial z}$$
\end{observación}

\begin{teorema}[Teorema de la divergencia de Gauss]
    Sea $A \subset \mathbb{R}^3$ un conjunto abierto y actotado, denotamos $V=\overline{A}$.\\
    Supongamos ahora que $\partial V = \partial A = S$ es una superficie cerrada, simple-regular a trozos, orientada con la normal $\vec{n}$ exterior a $V$.\\
    Sea ahora $\vec{F} : U \to \mathbb{R}^3$ un campo vectorial de clase $C^1$ definido en un abierto $U \supset V$. Entonces, si denotamos $S = \partial V$, se cumple la siguiente igualdad:
    \[ \int_V div(\vec{F}) = \int_{(S, \vec{n})} \vec{F} = \int_{\partial V} \vec{F}\]
\end{teorema}

\begin{observación}
    Aqui $\partial V = S = Fr(V)$ es la frontera topológica de $V$.
\end{observación}

\ejemplo{
    Verifiquemos el teorema para el siguiente cilindro:
    $$ V = \left\{ (x,y,z) \in \mathbb{R}^3 : x^2 + y^2 \leq 1, \quad 0 \leq z \leq 2 \right\}$$
    $$ \vec{F}(x,y,z) = (xy^2,x^2y,z)$$
    Tenemos que $\partial V = S_0 \cup S_1 \cup S_2$, donde:
    \begin{enumerate}
        \item[$S_0$:] La base inferior del cilindro
        $$S_0 = \{(x,y,z) \in \mathbb{R}^3 : x^2 + y^2 \leq 1, \quad z = 0\}$$
        \item[$S_1$:] La superficie exterior del cilindro
        $$S_1 = \{(x,y,z) \in \mathbb{R}^3 : x^2 + y^2 = 1, \quad 0 \leq z \leq 2 \}$$
        \item[$S_2$:] La base superior del cilindro
        $$S_2 = \{(x,y,z) \in \mathbb{R}^3 : x^2 + y^2 \leq 1, \quad z = 2\}$$
    \end{enumerate}
    Ahora tenemos que la divergencia es:
    $$div \vec{F} = \frac{\partial F_1}{\partial x} + \frac{\partial F_2}{\partial y} + \frac{\partial F_3}{\partial z} = \frac{\partial (xy^2)}{\partial x} + \frac{\partial (x^2y)}{\partial y} + \frac{\partial z}{\partial z} = y^2 + 2xy + 1$$
    Entonces podemos empezar a integrar
    $$\int_V div \vec{F} = \int_V (y^2 + 2xy + 1) dx dy dz =_{\text{cambiando a cilindricas}} \int_{z=0}^{z=2} \int_{\theta =0}^{\theta=2\pi} \int_{r=0}^{r=1} r \cdot \left(r^2+1\right) dr d\theta dz=$$
    $$=4\pi \int_{r=0}^{r=1} r^3 + r dr=4\pi \left[ \frac{r^4}{4} + \frac{r^2}{2} \right]_{r=0}^{r=1} = 3\pi$$
    Ahora aplicando el teorema de la divergencia de Gauss:
    $$\int_{(\partial V, \vec{n})} \vec{F} = \int_{(S_0, \vec{n})} \vec{F} + \int_{(S_1, \vec{n})} \vec{F} + \int_{(S_2, \vec{n})} \vec{F}$$
    Para $S_1$:
    $$ S_1 \to \varphi \begin{cases}
        x = \cos(\theta) \\
        y = \sin(\theta) \\
        z = z
    \end{cases} \qquad
    ...
    %$$ \int_{(S_1, \vec{n})} \vec{F} = \int_{D} \langle \vec{F} \circ \varphi_1, \vec{N}_\varphi_1 \rangle = \int_{\theta = 0}^{\theta = 2\pi} \int_{z=0}^{z=2} = \pi$$
    Para $S_0$:
...
\begin{proof}[Demostracion del teorema de la divergencia de Gauss para dominios proyectables]
...
\ejemplo{
    ...
    Asi el area de $S$ es:
    $$ \text{área}(S) = \int_{D} \lVert \vec{N}_\varphi \rVert dx dy = \int_{D} \sqrt{4x^2 + 4y^2 + 1} dx dy = \int_{\theta = 0}^{\theta = \frac{\pi}{2}} \frac{1}{8} \int_{r=0}^{r=1} 8r \sqrt{4r^2 + 1} dr d\theta =$$
