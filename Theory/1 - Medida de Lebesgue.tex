...
\ejemplo{
    ...
    \begin{center}
        \includegraphics[width=0.6\textwidth]{images/inteParab.png}
    \end{center}
...
\ejemplo{
    Sea $\gamma: [0, 2\pi] \to \mathbb{R}^2$ con $\gamma(t) = (\cos(t), \sin(t))$\\
    Con ademas: $\gamma(0) = (1,0) = p$ \\
    De lo que sacamos: $\gamma'(t) = (-\sin(t), \cos(t))$ \\
    En particular $\gamma'(0) = (0,1) = \vec{v}$ \\

    Cambiando de parámetro: \\
    Sea: $t=2\pi s$ con $0\leq s \leq 1$ \\
    Ahora: $\sigma(s) = (\cos(2\pi s), \sin(2\pi s))$ con $0\leq s \leq 1$ \\
    Entonces la derivada sale:
    $$
    \sigma'(s) = (2\pi (-\sin(2\pi s), \cos(2\pi s)))
    $$
    En particular, en $0$: $\sigma'(0) = 2\pi (0,1) = (0,2\pi)$\\

    Podemos hacer tambien el cambio $t=-2\pi s$ \\
    Que resulta: $\alpha(s) = (\cos(2\pi s), -\sin(2\pi s))$
    Entonces la derivada sale:
    $$
    \alpha'(s) = (2\pi (-\sin(2\pi s), -\cos(2\pi s)))
    $$
}
...
\begin{definición}
    ...
    Ademas $$\l(\gamma) = \int_{a}^{b} ||\gamma'(t)||dt = \int_{a}^{b} ds$$
\end{definición}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Haciendo ahora el cambio $t=h(s)$ y $dt=h'(s)$ obtenemos:
        $$\int_{s=c}^{s=d} f(\gamma(h(s)))$$
        FALTA POR TERMINAR
        \item Caso 2: $h$ es decreciente $(h'<0)$\\
        Tenemos $$ \int_{\gamma} f= \int_{t=a}^{t=b} f(\gamma(t)) ||(\gamma'(t))||dt=\int_{s=d}^{s=c}f(\gamma(h(s)))$$
        FALTA POR TERMINAR
    \end{enumerate}
\end{proof}
