...
\label{lema2Tonelli}
\begin{lema}
    Sea $(f_j)_{j \in \mathbb{N}}$ sucesión de funciones que satisfacen el Teorema de Tonelli y $f_j \uparrow f$ en $\mathbb{R}^{n+k}$ puntalmente $\implies$ f satisface el Teorema de Tonelli.
\end{lema}
\begin{proof}
    \leavevmode
    \begin{enumerate}
...
$\mathbb{R}^{n+k} = \mathbb{R}^n \times \mathbb{R}^k$. Con $Q = A \times B$ tal
que $A \subset \mathbb{R}^n$ y $B \subset \mathbb{R}^k$.
\begin{enumerate}
\item Sea $f_x = (\chi_Q)_x$ entonces:
\[
    \chi_Q(x,y) = \begin{cases}
        \chi_B & x \in A    \\
        0      & x \notin A
    \end{cases} \text{ luego es una función medible}
\]
Nótese que $(\chi_E)_x = \chi_{E_x}$, pues $(\chi_E)_x(y) = \chi_E(x, y) = \begin{cases} 1 & (x, y) \in E \iff y \in E_x \\ 0 & (x, y) \notin E \iff y \notin E_x \end{cases} = \chi_{E_x}(y)$.
\item $$F(x) = \int_{\mathbb{R}^k} (\chi_Q)_x = \begin{cases}
    ...
    \end{cases} = \begin{cases}
        ...
        0      & x \notin A
    \end{cases}$$
Luego $F(x) = m_k(B) \cdot \chi_A(x)$ es medible.
\item $\int_{\mathbb{R}^{n+k}}\chi_Q = m_{n+k}(Q) = m_n(A) \cdot m_k(B) = \int_{\mathbb{R}^n}m_k(B) \cdot \chi_A(x) = \int_{\mathbb{R}^n}F(x)$
\end{enumerate}
\item Supongamos que $f = \chi_G$ donde $G$ es abierto de $\mathbb{R}^{n+k}$. Sabemos
que $G = \bigcup_{i \in \mathbb{N}}Q_i$ con $Q_i$ cubos semiabiertos disjuntos.
Entonces $\forall j \in \mathbb{N}$ sea $G_j = \bigcup_{i = 1}^{j}Q_i$ tenemos que $(G_j)\uparrow G$ y $\chi_{G_j} \uparrow \chi_G$ con cada $\chi_{G_j} = \sum_{i
        = 1}^{j}\chi_{Q_i}$ que satisface el Teorema de Tonelli por lema 1 y (a).
Luego $\chi_G$ satisface el Teorema de Tonelli por el Lema 2.
...
\end{observación}
Entonces $D = \bigcap_{j = 1}^{\infty}G_j$ donde cada $G_j$ es es un conjunto abierto y acotado. Podemos suponer que $(G_j) \downarrow D$ por tanto $\chi_{G_j} \downarrow \chi_{D}$ y además, $0 \leq \chi_{G_j} \leq \chi_{G_{1}}$ que es integrable por ser acotada. Ahora si, podemos usar el Lema 2' para obtener que $\chi_{D}$ satisface el Teorema de Tonelli (pues $\chi_{G_1}$ es medible y satisface el Teorema de Tonelli por (2)).
\end{enumerate}
\end{proof}
