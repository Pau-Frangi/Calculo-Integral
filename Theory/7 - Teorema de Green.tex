...
pero podemos ver que $\vec{F}$ no es conservativo en $U$ ya que tomando $\gamma_r(t) = (r\cos(t), r\sin(t))$ con $r > 0$ y $t \in [0,2 \pi]$ tenemos que 
$$ \int_{\gamma} \vec{F} = \int_{0}^{2\pi} \left\langle \left(\frac{-r\sin(t)}{r^2}, \frac{r\cos(t)}{r^2}\right), \left(-r\sin(t), r\cos(t)\right) \right\rangle dt$$ 
$$= \int_{0}^{2\pi} \cos^2(t) + \sin^2(t) dt = \int_{0}^{2\pi} 1 dt = 2\pi \neq 0$$
luego como $\gamma_r$ es cerrada y $\int_{\gamma_r} \vec{F} \neq 0$ entonces $\vec{F}$ no es conservativo en $U$.\\
Sabemos que $\vec{F}$ no es conservativo, pero nos podemos preguntar si se puede encontrar un potencial para $\vec{F}$ en $U$. 
...
\begin{teorema} [Teorema de Green General - Dominios Múltiplemente Conexos]
    ...
    \end{enumerate}
    Sean $D = Int(C_0) \cap Ext(C_1) \cup \ldots \cup Ext(C_m)$ y $\vec{F} = (P, Q) : U \to \mathbb{R}^2$ campo vectorial de clase $C^1$ definido en un abierto $U \supset \overline{D} = D \cup C_0 \cup C_1 \cup \ldots \cup C_m$. Entonces:
