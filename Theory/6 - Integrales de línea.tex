    ...
    \begin{enumerate}
        ...
        \item Como $t \to \lVert \gamma'(t) \rVert $ es continua en casi todo $[a,
              ...
              $$ = \lVert \int_{t_{i -1}}^{t_i}(\gamma'(t_i) - \gamma'(t))dt
                  +\int_{t_{i-1}}^{t_i} \gamma'(t)dt \rVert + \varepsilon(t_i - t_{i-1}) $$
              Aplicando la desigualdad triangular: $$\leq \lVert
                  \int_{t_{i-1}}^{t_i}(\gamma'(t_i) - \gamma'(t))dt \rVert + \lVert
                  \int_{t_{i-1}}^{t_i} \gamma'(t)dt \rVert + \varepsilon(t_i - t_{i-1})$$ Por el
              \cref{normaIntegrales} se sigue que: $$ \leq \int_{t_{i-1}}^{t_i}
                  \underbrace{\lVert \gamma'(t_i) - \gamma'(t) \rVert}_{< \varepsilon} dt + \lVert
                  \int_{t_{i-1}}^{t_i} \gamma'(t) dt \rVert + \varepsilon(t_i - t_{i-1}) \leq
                  2\varepsilon(t_i - t_{i-1}) + \lVert \int_{t_{i-1}}^{t_i} \gamma'(t) dt \rVert$$
              $$ = 2\varepsilon(t_i - t_{i-1}) + \lVert \gamma(t_i) - \gamma(t_{i-1}) \rVert$$
              Luego, $$\int_{a}^{b} \lVert \gamma'(t) \rVert dt = \sum_{i = 1}^n
                  \int_{t_{i-1}}^{t_i} \lVert \gamma'(t) \rVert dt \leq \sum_{i =
                      1}^{n}2\varepsilon(t_i - t_{i-1}) + \lVert \gamma(t_i) - \gamma(t_{i-1}) \rVert $$
              $$= 2\varepsilon(b-a) + \Sigma(\gamma, \mathcal{P}) \leq 2\varepsilon(b-a) +
                  l(\gamma)$$
    \end{enumerate}
\end{proof}
